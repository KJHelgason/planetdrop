# multi_cloud_2.gd
# This script controlls the computer cloud(Spaceship) behvaior in versus mode level 1
...
func _input(event):
	# Begin to drop the ball when spacebar is pressed and Computer is ready
	if event.is_action_pressed("drop_ball") and current_ball and !is_dropping and ScoreManager.ai_ready:
		
		# Instantly teleport AI to new X position
		ai_target_x = randf_range(x_min, x_max)
		print("Teleporting to:", ai_target_x)
		position.x = clamp(ai_target_x, x_min, x_max)

		# short reaction time - Computer drops ball 0.3 seconds after human
