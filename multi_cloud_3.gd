# multi_cloud_3.gd
# This script controlls the computer cloud(Spaceship) behvaior in versus mode level 2
...
# Process Ball Dropping with Computer Logic for 66% accurate drop rate
func _input(event):
	if event.is_action_pressed("drop_ball") and current_ball and !is_dropping and ScoreManager.ai_ready:
		# 66% chance to do a smart drop
		if rng.randf() < 0.66:
			# Smart drop aligns with closest same-kind ball
			var best_match: RigidBody2D = null
			var best_distance := INF
			var balls = get_tree().get_nodes_in_group("balls")
			
			# Find and calculate best_match [same level, and distance]
			for ball in balls:
				if ball == current_ball or ball == next_ball:
					continue
				if ball.get_level() == current_ball.get_level():
					var distance = abs(ball.global_position.x - position.x)
					if distance < best_distance:
						best_distance = distance
						best_match = ball

			# If match found, set X position to it
			if best_match:
				ai_target_x = clamp(best_match.global_position.x, x_min, x_max)
			else:
				# fallback to random drop if no match found
				ai_target_x = randf_range(x_min, x_max)
		else:
			# Normal drop: random drop location 
			ai_target_x = randf_range(x_min, x_max)
